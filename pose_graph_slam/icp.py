...
def icp(reference_points, points, max_iterations=100, distance_threshold=0.3, convergence_translation_threshold=1e-5,
        convergence_rotation_threshold=1e-5, point_pairs_threshold=30, verbose=False):
    """
    ...
    for iter_num in range(max_iterations):
        ...
        for nn_index in range(len(distances)):
            # if indices[nn_index, 0] not in used_ind:
            if distances[nn_index][0] < distance_threshold:
                closest_point_pairs.append((points[nn_index], reference_points[indices[nn_index][0]]))
                # used_ind.add(indices[nn_index, 0])
